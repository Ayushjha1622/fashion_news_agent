...
def high_impact():
    ...
    articles = list(db.articles.find({"impact": "HIGH"}, {"_id": 0}))
...
def latest_articles():
    ...
    articles = list(db.articles.find({}, {"_id": 0}).sort("createdAt", -1).limit(10))
...
def stats():
    ...
    high = db.articles.count_documents({"impact": "HIGH"})

    medium = db.articles.count_documents({"impact": "MEDIUM"})

    low = db.articles.count_documents({"impact": "LOW"})
...
def competitor_news():
    ...
    articles = list(
        db.articles.find(
            {"competitorsMentioned": {"$exists": True, "$ne": []}}, {"_id": 0}
        )
    )
